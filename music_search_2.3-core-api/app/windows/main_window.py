...
class MainWindow(QMainWindow):
    ...
    def on_search(self, query: str, limit: int, mode: str):
        ...
        try:
            ...
            # --- SICHERHEITS-CHECK FÜR CORE ---
            # Wenn wir Context suchen, müssen wir sicherstellen, dass der Key da ist.
            # Da wir hier im Main-Thread sind, dürfen wir Dialoge öffnen.
            if domain in (SearchDomain.ALL, SearchDomain.CONTEXT):
                if not corelib.ensure_api_key(self):
                # Wenn der User "Abbrechen" drückt, suchen wir trotzdem, 
                # aber halt ohne CORE Ergebnisse (oder brechen ganz ab, wie du willst).
                    print("DEBUG: User hat CORE Key Eingabe abgebrochen.")
                    # Falls du die Suche dann komplett stoppen willst: return
                    # Falls du nur CORE überspringen willst, lass es weiterlaufen.
